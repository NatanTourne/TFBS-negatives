...
if __name__ == "__main__":
    ...
    Dmod = DataModule(file, TF=args.TF, batch_size=args.batch_size, neg_mode=args.neg_mode, cross_val_set=args.cross_val_set, max_positives=args.pos_limit)
    model = TFmodel(
    ...
    checkpoint_callback = ModelCheckpoint(
            ...
            filename=args.celltype + "_" + args.TF + "_" + args.neg_mode +"_CV-" + str(args.pos_limit)+"_pos-"+ str(args.cross_val_set) + "_" + date + '_{epoch:02d}_{val_loss:.2f}_{AUROC:.2f}',
            mode="max"
            )
    ...
    wandb_logger = WandbLogger(project="Negatives_final", entity="ntourne", config=vars(args), name=run_name, group=args.group_name)
