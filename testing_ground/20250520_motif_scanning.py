            ...
            if matrices == []:
                ...
                for i in range(dataset.__len__()): 
                    ...
                    for mid in matrices:
                        ...
                        results.setdefault(mid, []).append(best_score)

            filtered_jaspar_dict = {key: value for key, value in jaspar_dict.items() if value and key in TF_list}
            filtered_jaspar_dict_reverse = {}
            for tf, ids in filtered_jaspar_dict.items():
                for matrix_id in ids:
                    filtered_jaspar_dict_reverse[matrix_id] = tf

        AUROC_scores = {}
        accuracy_scores = {}

        for mid, scores in results.items():
            labels = true_vals[filtered_jaspar_dict_reverse[mid]]
            scores = np.array(scores)
            labels = np.array(labels)
            
            if len(scores) != len(labels):
                print(f"Length mismatch for {mid}: {len(scores)} vs {len(labels)}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            if len(np.unique(labels)) < 2:
                print(f"Only one class present for {mid}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            if len(scores) == 0:
                print(f"No scores for {mid}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            if len(labels) == 0:
                print(f"No labels for {mid}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            if np.isnan(scores).any():
                print(f"NaN values in scores for {mid}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            if np.isnan(labels).any():
                print(f"NaN values in labels for {mid}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            if len(np.unique(labels)) == 1:
                print(f"Only one class present in labels for {mid}")
                accuracy_scores[mid] = None
                AUROC_scores[mid] = None
                continue
            ...
            try:
                ...
                AUROC_scores[mid] = auc
            except ValueError as e:
                print(f"ValueError for {mid}: {e}")
                continue
            ...
            try:
                ...
                accuracy_scores[mid] = acc
            except Exception as e:
                print(f"Error calculating accuracy for {mid}: {e}")
                continue
        filtered_jaspar_values = [item for sublist in filtered_jaspar_dict.values() for item in sublist]
        filtered_jaspar_values
        for TF in filtered_jaspar_dict.keys():
            matrices = filtered_jaspar_dict[TF]
            temp_auroc = [AUROC_scores[i] for i in matrices if AUROC_scores[i] is not None]
            if temp_auroc == []:
                auroc = None
            else:
                auroc = np.max(temp_auroc)
            temp_accuracy = [accuracy_scores[i] for i in matrices if accuracy_scores[i] is not None]
            if temp_accuracy == []:
                accuracy = None
            else:
                accuracy = np.max(temp_accuracy)
                
            tf_auroc_scores.setdefault(TF, []).append(auroc)
            tf_accuracy_scores.setdefault(TF, []).append(accuracy)

    df = pd.DataFrame.from_dict(
        {tf: tf_auroc_scores[tf] + tf_accuracy_scores[tf] for tf in tf_auroc_scores.keys()},
        orient='index',
