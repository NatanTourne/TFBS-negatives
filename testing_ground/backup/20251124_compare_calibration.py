...
def get_predicts(model_ckpt_path, device=3, idx=0):
    ...
    # filename="CT-" + args.celltype + "_TF-" + args.TF.replace('_', '$') + "_NEG-" + args.neg_mode +"_CV-" + str(args.cross_val_set) + "_LR-"+ str(args.learning_rate) + "_Date-" + date + '_{epoch:02d}-{val_loss:.2f}-{AUROC:.2f}',
    cellline = parts[0].split('-')[1] 
    tf = parts[1].split('-')[1].replace('$', '_')
    neg_mode = parts[2].split('-')[1].replace('-', '_')
    cv_split = parts[3].split('-')[1]
    ...
    Dmod = DataModule(cellline_file, TF=tf, batch_size=256, neg_mode=neg_mode, cross_val_set=int(cv_split))
...
for filename in ckpt_files:   
    ...
    data.append({
        ...
        'cellline': cellline,
        'TF': tf,
        'negative_type': neg_mode,
        'cv_split': cv_split,
        "learning_rate": parts[4].split('-')[1]
    })
