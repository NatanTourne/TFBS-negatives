...
def get_predicts(model_ckpt_path, device=3, idx=0):
    ...
    # Split on '_' (filenames still use '_' as separators; '_' inside TF/neg_mode were replaced with '$')
    ...
    if cellline is None or tf is None or neg_type is None or cv_split is None:
    ...
    model = SimpleTFmodel.load_from_checkpoint(model_ckpt_path)
...
folder_path = "/data/home/natant/Negatives/Runs/Review_calibration/LR"
...
output_folder = "/data/home/natant/Negatives/Runs/Review_calibration/LR/plots"
...
for TF in TF_list:
    ...
    for cross_val in range(5):
        ...
        for neg_type in neg_type_order:
            try:
                ...
                print(f"\033[91mError processing TF: {TF}, CV: {cross_val}, NEG: {neg_type}. Error: {e}\033[0m")
                continue
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder,  f'{cellline}_{TF}_HQ_dataset_calibration.png'), dpi=150, bbox_inches='tight')
